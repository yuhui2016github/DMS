...
import com.example.yuhui.dms.dmscatalogue.adapter.AdapterDataObserverImpl;
import com.example.yuhui.dms.dmscatalogue.adapter.WrapAdapter;
 ...
 */
public class ProductsBrowsingView extends RecyclerView implements LoadingMoreFooterClickCallback {

    private static final String TAG = "ProductsBrowsingView";
    ...
    private ArrayList<LoadingMoreFooterView> mFootViews = new ArrayList<>();//尾部view的集合
    private RecyclerView.Adapter mAdapter;//里层的adapter

    private float mLastY = -1;
    private int pageSize = 10;
    private int visibleThreshold = 2; // list到达 最后一个item的时候 触发加载
    private LoadingListener mLoadingListener;
    private RefreshHeaderView mRefreshHeader;//header view
    ...
    public ProductsBrowsingView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public ProductsBrowsingView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context) {
        ...
        RefreshHeaderView refreshHeader = new RefreshHeaderView(context);
        mHeaderViews.add(0, refreshHeader);
        mRefreshHeader = refreshHeader;
        //添加一个footer view 用于上拉加载
        LoadingMoreFooterView footView = new LoadingMoreFooterView(context);
     ...
     * @param view
     */
    public void addFootView(LoadingMoreFooterView view) {
        mFootViews.clear();
        mFootViews.add(view);
    }
    ...
    public void onClick(View view) {
        if (mLoadingListener != null && loadingMoreEnabled && !isRefreshing()) {
            LoadingMoreFooterView footView = mFootViews.get(0);
            footView.loading();
        ...
        if (pullRefreshEnabled) {
            ...
            switch (ev.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    mLastY = ev.getRawY();
                    if (isOnTop()) {
                        Log.i(TAG, " onMove deltaY :" + deltaY);
